...
export default function AdminProductsPage() {
  ...
  const fetchProducts = useCallback(async () => {
    setLoading(true)
    const { data } = await (supabaseAdmin as any).from('products').select('*').order('created_at', { ascending: false })
    const prods = data || []
    setProducts(prods)
    // Mini analytics
  ...
  async function toggleActive(id: string, current: boolean) {
    await (supabaseAdmin as any).from('products').update({ is_active: !current }).eq('id', id)
    setProducts(prev => prev.map(p => p.id === id ? { ...p, is_active: !current } : p))
  }

  async function deleteProduct(id: string, name: string) {
    if (!confirm(`هل أنت متأكد من حذف "${name}"؟`)) return
    await (supabaseAdmin as any).from('products').delete().eq('id', id)
